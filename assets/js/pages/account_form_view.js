...
/**
 * Account form view page.
 *
 * Displays a form and allows completion.
 */
App.Pages.AccountFormView = (function () {
    const $title = $('#account-form-title');
    const $content = $('#account-form-content');
    const $fields = $('#account-form-fields');
    const $submit = $('#account-form-submit');
    const $status = $('#account-form-status');
    const $message = $('#account-form-message');
    ...
    function renderFieldInput(field, value = '', readOnly = false) {
        ...
        if (fieldType === 'dropdown') {
            ...
            options.forEach((option, index) => {
                const optionId = `account-form-radio-${fieldId}-${index}`;
                const $wrap = $('<div/>', { class: 'form-check' });
                const $radio = $('<input/>', {
                    type: 'radio',
                    id: optionId,
                    name: `account-form-${fieldId}`,
            ...
            $input = $('<div/>', { class: 'd-flex flex-column gap-2' });
            options.forEach((option, index) => {
                const optionId = `account-form-checkbox-${fieldId}-${index}`;
                const $wrap = $('<div/>', { class: 'form-check' });
    ...
    function onSubmit() {
        ...
        App.Http.Forms.submit(currentForm.id, responses)
            .done(() => {
    ...
    function loadForm() {
        ...
        App.Http.Forms.findForUser(formId)
            .done((form) => {
                currentForm = form;
